...
def primary_key_column_names_renamed(df:DataFrame, primary_key_prev:list[str], primary_key_new:list[str]):
    if primary_key_new is None:
        return df
    
    if len(primary_key_prev) != len(primary_key_new):
        raise ValueError("primary_key_prev and primary_key_new must have the same length.")
